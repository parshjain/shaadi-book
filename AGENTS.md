...
This project is developed using Codex as an orchestrator with Arcanist coding agents.
...
Every Codex session on this repo MUST use the workbook to coordinate with other sessions.

### On session start:
1. Generate your agent name: run `~/.Codex/scripts/workbook-init.sh` to get a city-based ID
...
1. Acquire lock: `~/.Codex/scripts/workbook-lock.sh`
2. Read the file, make edits, write back
3. Release lock: `~/.Codex/scripts/workbook-unlock.sh`
